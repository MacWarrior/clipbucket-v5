	...
	class CBEmail
	{
		...
		function update_template($params)
		{
			...
			else
			{
				$db->update(
					tbl($this->db_tpl),
					array("email_template_subject","email_template"),
					array($subj,'|no_mc|'.$msg),
		...
		function send_emails($id)
		{
			...
			if(!$users)
			{
				...
				foreach($users as $user)
				{
					...
					cbmail(array('from_name'=>TITLE, 'to'=>$user['email'],'from'=>$email['email_from'],'subject'=>$subj,'content'=>$msg));
					$sent++;
