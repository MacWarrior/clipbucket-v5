...
class Automate
{
    ...
    public static function getAll(): array
    {
        ...
        $sql = /** @lang MySQL */ 'WITH main AS ( 
                        ...
                        SELECT at.id_automate_task, (@id_automate_task := at.id_automate_task),
                               ( SELECT ah.id_automate_histo
                                 FROM ( SELECT ah_1.id_automate_histo,
                                        ...
                                        WHERE @id_automate_task = ah_1.id_automate_task) ah
                               ...
                               ( SELECT ah.id_automate_histo
                                 FROM ( SELECT ah_1.id_automate_histo,
                                               ...
                                               row_number() OVER (PARTITION BY ah_1.id_automate_task ORDER BY ah_1.date_debut DESC) AS pos
                                        FROM ' . tbl('automate_histo').' ah_1
                                        WHERE @id_automate_task = ah_1.id_automate_task AND ah_1.id_automate_statut = 1) ah
               ...
               WHERE  AT.id_automate_task_type = 1 /* periodic */ 
               ...
               WHERE ( 
                    AT.id_automate_task_type = 2 /* one shot */ 
               )
        ...
        foreach ($rs as $k => $v){
            $rs[$k]['frequence_trad'] = (CronSchedule::fromCronString($v['frequence'], 'fr'))->asNaturalLanguage();
        }
        return $rs;
    }
