...
class myquery
{
    ...
    {
        return $this->VideoExists($videokey);
    }
    ...
    {
        return $this->delete_comment($videoid);
    }
    ...
    /**
     * Add like for ClipBuckets
     * 
     * @param ARRAY $params
     * @return BOOLEAN 
     */
    function add_like($array)
    {
        global $db;
        
        $is_liked = $this->is_liked($array);
        ...
        if($is_liked)
        {
            if($is_liked['liked']!=$liked)
                $this->update_like($is_liked['like_id'],$liked);
            return true;
        }
        ...
        $insert_id = db_insert(tbl('likes'), $fields);
    ...
    function add_dislike($array)
    {
        $array['liked'] = 'no';
        return $this->add_like($array);
    }
    
    /**
     * Function used to dislike a video
     * 
     * @param ARRAY 
     */

    /**
     * function used to unlike object , removes the entry..
     * 
     * @param ARRAY 
     */
    function remove_like($array)
    {
        global $db;
        ...
        $db->execute("DELETE FROM ".tbl('feeds')." WHERE ".$cond);
        
        return true;
    }
    
    /**
     * check if user has already liked the object or not
     */
    function is_liked($array,$liked=NULL)
    {
        global $db;
        ...
        $results = db_select("SELECT FROM ".tbl('feeds')." WHERE ".$cond);
        $results = $results[0];
     ...
     * Update liked
     */
    function update_like($like_id,$liked='yes')
    {
        global $db;
        ...
        db_update(tbl('likes'),$fields," like_id='$like_id' ");
