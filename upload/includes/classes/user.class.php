...
class User extends Objects
{
    ...
    public function delete(): void
    {
        ...
        Clipbucket_db::getInstance()->execute('UPDATE ' . tbl('flags') . ' SET userid=\'' . $anonymous_id . '\' WHERE userid=' . (int)$user_id);
        // Delete categories
        ...
        if (config('delete_comments_on_delete_user') == 'yes') {
            ...
            Clipbucket_db::getInstance()->execute('UPDATE ' . tbl('comments') . ' SET userid=\'' . $anonymous_id . '\' WHERE userid=' . (int)$uid);
        }
        ...
        Clipbucket_db::getInstance()->execute('UPDATE ' . tbl('messages') . ' SET message_from=\'' . userquery::getInstance()->get_anonymous_user() . '\' WHERE message_from=' . (int)$uid);
...
class userquery extends CBCategory
{
    ...
    {
        if (!empty($_FILES['background_file']['name'])) {
            $file = Upload::getInstance()->upload_user_file('background', $_FILES['background_file'], User::getInstance()->getCurrentUserID());
            if ($file) {
                $uquery_field = ['background'];
                $uquery_val = [$file];
                $log_array = [
    ...
    public function updateBackground(array $data = []): array
    {
        ...
        if ($av_details[0] > config('max_bg_width')) {
            ...
            return [
                'status' => false,
                'msg'    => lang('file_width_exceeds', config('max_bg_width'))
            ];
        }
