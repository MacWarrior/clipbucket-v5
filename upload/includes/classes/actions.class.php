...
class cbactions
{
    ...
    function builtin_plugins($userid = NULL, $autocreate = false)
    {
        $userid = mysql_clean($userid);
        $query = "SELECT playlist_id,category FROM " . tbl("playlists");
        $query .=" WHERE category !='normal' ";
        $query .= "AND userid='" . $userid . "'";
        
        $query .= "  LIMIT 5 ";

        $playlists = db_selecT($query);
        ...
        if ($playlists)
        {
            ...
            foreach ($playlists as $playlist)
            {
                ...
                $the_playlists[$playlist['category']] = $playlist;
            }
            

            foreach ($cb_playlists as $cb_playlist)
            {
                if (!in_array($cb_playlist, $categories))
                {
                    $playlist_arr = array(
                        'name' => $cb_playlist,
                        'category'  => $cb_playlist,
                        'log_messages' => 'no'
                    );

                    $pid = $this->create_playlist($playlist_arr);
                    
                    $the_playlists[$cb_playlist] = array(
                        'playlist_id'   => $pid,
                        'category'      => $cb_playlist
                    );
                }
        ...
        else
        {
            if ($userid && $autocreate)
            {
                ...
                foreach ($cb_playlists as $playlist)
                {
                    $playlist_arr = array(
                        'name' => $playlist,
                        'category'  => $playlist,
    ...
    function report_it($id)
    {
        ...
        if ($this->exists($id))
        {
            if (userid())
            {
                ...
                {
                    $db->insert(tbl($this->flag_tbl), array('type', 'id', 'userid', 'flag_type', 'date_added'), array($this->type, $id, userid(), mysql_clean(post('flag_type')), NOW()));
