ALTER TABLE `{tbl_prefix}video_subtitle`
    DROP FOREIGN KEY IF EXISTS `{tbl_prefix}video_subtitle_ibfk_1`;
ALTER TABLE `{tbl_prefix}video_subtitle`
    ADD CONSTRAINT `video_subtitle_ibfk_1` FOREIGN KEY IF NOT EXISTS (`videoid`) REFERENCES `cb_video`(`videoid`) ON DELETE CASCADE ON UPDATE CASCADE;

ALTER TABLE `{tbl_prefix}languages_translations`
    DROP FOREIGN KEY IF EXISTS `{tbl_prefix}languages_translations_ibfk_1`,
    DROP FOREIGN KEY IF EXISTS `{tbl_prefix}languages_translations_ibfk_2`;
...
ALTER TABLE `{tbl_prefix}video_thumbs`
    DROP FOREIGN KEY IF EXISTS `{tbl_prefix}video_thumbs_ibfk_1`;
