INSERT INTO `{tbl_prefix}languages_keys` (language_key)
VALUES ('ad_name_error'),
       ('ad_exists_error1'),
       ('ad_exists_error2'),
       ('ad_add_msg'),
       ('ad_msg'),
       ('ad_update_msg'),
       ('ad_del_msg'),
       ('ad_deactive'),
       ('ad_active'),
       ('ad_placment_delete_msg'),
       ('ad_placement_err1'),
       ('ad_placement_err2'),
       ('ad_placement_err3'),
       ('ad_placement_msg'),
       ('cat_exist_error'),
       ('cat_add_msg'),
       ('cat_update_msg'),
       ('plugin_install_msg'),
       ('plugin_no_file_err'),
       ('plugin_file_detail_err'),
       ('plugin_installed_err'),
       ('plugin_no_install_err'),
       ('grp_comment_msg'),
       ('usr_cpass_err'),
       ('usr_pass_err'),
       ('usr_cpass_err1'),
       ('usr_cmt_del_msg'),
       ('usr_sub_err'),
       ('usr_exist_err'),
       ('usr_uname_err'),
       ('usr_uname_err2'),
       ('usr_pass_err2'),
       ('usr_email_err1'),
       ('usr_email_err2'),
       ('usr_email_err3'),
       ('usr_uname_err3'),
       ('usr_pass_err3'),
       ('usr_ament_err'),
       ('usr_reg_err'),
       ('usr_ban_err'),
       ('usr_login_err'),
       ('usr_sub_msg'),
       ('usr_uname_email_msg'),
       ('usr_rpass_email_msg'),
       ('usr_pass_email_msg'),
       ('usr_del_msg'),
       ('usr_ac_msg'),
       ('usr_dac_msg'),
       ('usr_uban_msg'),
       ('usr_uuban_msg'),
       ('usr_upd_succ_msg'),
       ('usr_activation_msg'),
       ('usr_activation_err'),
       ('usr_activation_em_msg'),
       ('usr_pof_upd_msg'),
       ('usr_arr_no_ans'),
       ('usr_arr_elementary'),
       ('usr_arr_hi_school'),
       ('usr_arr_some_colg'),
       ('usr_arr_assoc_deg'),
       ('usr_arr_bach_deg'),
       ('usr_arr_mast_deg'),
       ('usr_arr_phd'),
       ('usr_arr_post_doc'),
       ('usr_arr_single'),
       ('usr_arr_married'),
       ('usr_arr_comitted'),
       ('usr_arr_open_relate'),
       ('title_crt_new_msg'),
       ('title_usr_fav_vids'),
       ('vdo_iac_msg'),
       ('class_error_occured'),
       ('class_cat_del_msg'),
       ('class_vdo_del_msg'),
       ('class_vdo_fr_msg'),
       ('class_fr_msg1'),
       ('class_vdo_act_msg'),
       ('class_vdo_act_msg1'),
       ('class_vdo_update_msg'),
       ('class_subj_err'),
       ('class_vdo_del_err'),
       ('class_unsub_msg'),
       ('class_vdo_exist_err'),
       ('com_forgot_username'),
       ('com_manage_vids'),
       ('com_my_inbox'),
       ('com_my_channel'),
       ('com_send_message'),
       ('com_manage_fav'),
       ('com_manage_subs'),
       ('com_advance_results'),
       ('sort_by_most_viewed'),
       ('featured'),
       ('account'),
       ('all'),
       ('active'),
       ('activate'),
       ('deactivate'),
       ('by'),
       ('cancel'),
       ('categories'),
       ('category'),
       ('channels'),
       ('comments'),
       ('comment'),
       ('companies'),
       ('contact_us'),
       ('country'),
       ('date'),
       ('date_added'),
       ('delete'),
       ('add'),
       ('delete_selected'),
       ('duration'),
       ('education'),
       ('email'),
       ('embed_code'),
       ('favourites'),
       ('female'),
       ('friends'),
       ('from'),
       ('gender'),
       ('hobbies'),
       ('inbox'),
       ('joined'),
       ('location'),
       ('login'),
       ('logout'),
       ('male'),
       ('members'),
       ('messages'),
       ('message'),
       ('minute'),
       ('minutes'),
       ('sort_by_most_recent'),
       ('my_account'),
       ('next'),
       ('no'),
       ('optional'),
       ('password'),
       ('request'),
       ('reply'),
       ('second'),
       ('seconds'),
       ('send'),
       ('sent'),
       ('signup'),
       ('subject'),
       ('tags'),
       ('to'),
       ('type'),
       ('update'),
       ('upload'),
       ('videos'),
       ('website'),
       ('yes'),
       ('previous'),
       ('rating'),
       ('remove'),
       ('search'),
       ('services'),
       ('sort_by'),
       ('subscribers'),
       ('tag_title'),
       ('track_title'),
       ('username'),
       ('views'),
       ('mostly_viewed'),
       ('most_comments'),
       ('insufficient_privileges'),
       ('vdo_edit_vdo'),
       ('vdo_video_details'),
       ('vdo_title'),
       ('vdo_desc'),
       ('vdo_cat'),
       ('vdo_cat_msg'),
       ('vdo_br_opt'),
       ('vdo_br_opt1'),
       ('vdo_br_opt2'),
       ('vdo_add_eg'),
       ('vdo_share_opt'),
       ('vdo_allow_comm'),
       ('vdo_dallow_comm'),
       ('vdo_allow_rating'),
       ('vdo_embedding'),
       ('vdo_embed_opt1'),
       ('vdo_manage_vdeos'),
       ('vdo_status'),
       ('grp_view_vdos'),
       ('grp_manage_members_title'),
       ('usr_activation_title'),
       ('usr_actiavation_msg1'),
       ('usr_activation_code_tl'),
       ('usr_attach_video'),
       ('user_forgot_message'),
       ('user_veri_code'),
       ('user_recover'),
       ('user_reset'),
       ('user_inactive_msg'),
       ('user_fav_videos'),
       ('user_channel_profiles'),
       ('user_manage_my_account'),
       ('user_sent_box'),
       ('user_manage_contacts'),
       ('user_forgot_username'),
       ('all_fields_req'),
       ('user_allowed_format'),
       ('user_confirm_pass'),
       ('user_date_of_birth'),
       ('user_i_agree_to_the'),
       ('user_change_pass'),
       ('user_profile_settings'),
       ('user_fname'),
       ('user_lname'),
       ('user_relat_status'),
       ('user_about_me'),
       ('user_fav_movs_shows'),
       ('user_fav_music'),
       ('user_fav_books'),
       ('user_user_avatar'),
       ('user_change_email'),
       ('user_email_address'),
       ('user_new_email'),
       ('user_old_pass'),
       ('user_new_pass'),
       ('user_c_new_pass'),
       ('user_doesnt_exist'),
       ('user_s_channel'),
       ('user_last_login'),
       ('user_send_message'),
       ('user_add_comment'),
       ('menu_home'),
       ('menu_inbox'),
       ('vdo_cat_err2'),
       ('you_already_logged'),
       ('you_not_logged_in'),
       ('invalid_user'),
       ('vdo_cat_err3'),
       ('vid_thumb_changed'),
       ('upload_vid_thumbs_msg'),
       ('video_thumb_delete_msg'),
       ('video_thumb_delete_err'),
       ('no_comment_del_perm'),
       ('user_contains_disallow_err'),
       ('add_cat_erro'),
       ('add_cat_no_name_err'),
       ('cat_default_err'),
       ('pic_upload_vali_err'),
       ('cat_dir_make_err'),
       ('cat_set_default_ok'),
       ('vid_thumb_removed_msg'),
       ('vid_files_removed_msg'),
       ('vid_log_delete_msg'),
       ('vdo_multi_del_erro'),
       ('add_fav_message'),
       ('obj_not_exists'),
       ('already_fav_message'),
       ('obj_report_msg'),
       ('obj_report_err'),
       ('user_no_exist_wid_username'),
       ('share_video_no_user_err'),
       ('today'),
       ('yesterday'),
       ('thisweek'),
       ('lastweek'),
       ('thismonth'),
       ('lastmonth'),
       ('thisyear'),
       ('lastyear'),
       ('favorites'),
       ('alltime'),
       ('insufficient_privileges_loggin'),
       ('show_dob'),
       ('profile_tags'),
       ('online_status'),
       ('show_profile'),
       ('allow_ratings'),
       ('postal_code'),
       ('no_date_provided'),
       ('bad_date'),
       ('users_videos'),
       ('please_login_subscribe'),
       ('users_subscribers'),
       ('user_subscriptions'),
       ('usr_avatar_update'),
       ('user_email_confirm_email_err'),
       ('email_change_msg'),
       ('no_edit_video'),
       ('remove_fav_photo_confirm'),
       ('remove_fav_collection_confirm'),
       ('fav_remove_msg'),
       ('unknown_favorite'),
       ('vdo_multi_del_fav_msg'),
       ('unknown_sender'),
       ('please_enter_message'),
       ('unknown_reciever'),
       ('no_pm_exist'),
       ('pm_sent_success'),
       ('msg_delete_inbox'),
       ('msg_delete_outbox'),
       ('private_messags_deleted'),
       ('spe_users_by_comma'),
       ('user_ban_msg'),
       ('no_user_ban_msg'),
       ('thnx_sharing_msg'),
       ('no_comment_exists'),
       ('please_login_create_playlist'),
       ('play_list_with_this_name_arlready_exists'),
       ('please_enter_playlist_name'),
       ('new_playlist_created'),
       ('playlist_not_exist'),
       ('playlist_item_not_exist'),
       ('playlist_item_delete'),
       ('play_list_updated'),
       ('you_dont_hv_permission_del_playlist'),
       ('playlist_delete_msg'),
       ('playlist_name'),
       ('add_new_playlist'),
       ('this_already_exist_in_pl'),
       ('edit_playlist'),
       ('remove_playlist_item_confirm'),
       ('remove_playlist_confirm'),
       ('avcode_incorrect'),
       ('manage_playlist'),
       ('my_notifications'),
       ('users_contacts'),
       ('type_flags_removed'),
       ('users'),
       ('login_to_mark_as_spam'),
       ('no_own_commen_spam'),
       ('already_spammed_comment'),
       ('spam_comment_ok'),
       ('unable_find_download_file'),
       ('page_doesnt_exist'),
       ('pelase_select_img_file_for_vdo'),
       ('new_mem_added'),
       ('email_template_not_exist'),
       ('email_subj_empty'),
       ('email_msg_empty'),
       ('email_tpl_has_updated'),
       ('page_name_empty'),
       ('page_title_empty'),
       ('page_content_empty'),
       ('new_page_added_successfully'),
       ('page_updated'),
       ('page_deleted'),
       ('page_activated'),
       ('page_deactivated'),
       ('you_cant_delete_this_page'),
       ('ad_placement_err4'),
       ('thnx_for_voting'),
       ('please_login_to_rate'),
       ('you_not_subscribed'),
       ('you_cant_delete_this_user'),
       ('you_dont_hv_perms'),
       ('user_subs_hv_been_removed'),
       ('user_subsers_hv_removed'),
       ('you_already_sent_frend_request'),
       ('friend_added'),
       ('friend_request_sent'),
       ('friend_confirm_error'),
       ('friend_confirmed'),
       ('friend_request_not_found'),
       ('you_cant_confirm_this_request'),
       ('friend_request_already_confirmed'),
       ('user_no_in_contact_list'),
       ('user_removed_from_contact_list'),
       ('cant_find_level'),
       ('please_enter_level_name'),
       ('level_updated'),
       ('level_del_sucess'),
       ('level_not_deleteable'),
       ('pass_mismatched'),
       ('user_blocked'),
       ('user_already_blocked'),
       ('you_cant_del_user'),
       ('user_vids_hv_deleted'),
       ('user_contacts_hv_removed'),
       ('all_user_inbox_deleted'),
       ('all_user_sent_messages_deleted'),
       ('pelase_enter_something_for_comment'),
       ('please_enter_your_name'),
       ('please_enter_your_email'),
       ('template_activated'),
       ('error_occured_changing_template'),
       ('language_does_not_exist'),
       ('lang_added'),
       ('default_lang_del_error'),
       ('lang_deleted'),
       ('lang_name_empty'),
       ('lang_code_empty'),
       ('lang_updated'),
       ('player_activated'),
       ('error_occured_while_activating_player'),
       ('plugin_has_been_s'),
       ('plugin_uninstalled'),
       ('acitvation_html_message'),
       ('acitvation_html_message2'),
       ('account_details'),
       ('please_select_img_file'),
       ('or'),
       ('pelase_enter_image_url'),
       ('user_bg'),
       ('delete_this_img'),
       ('current_email'),
       ('confirm_new_email'),
       ('no_subs_found'),
       ('default'),
       ('date_recorded_location'),
       ('update_video'),
       ('remember_me'),
       ('notifications'),
       ('playlists');

INSERT INTO `{tbl_prefix}languages_keys` (language_key)
VALUES ('change_style_of_listing'),
       ('manage_playlists'),
       ('total_items'),
       ('play_now'),
       ('no_video_in_playlist'),
       ('view'),
       ('you_dont_hv_fav_vids'),
       ('private_messages'),
       ('new_private_msg'),
       ('signup_success_usr_ok'),
       ('signup_success_usr_emailverify'),
       ('report_this'),
       ('add_to_playlist'),
       ('view_profile'),
       ('subscribe'),
       ('more'),
       ('link_this_video'),
       ('name'),
       ('email_wont_display'),
       ('please_login_to_comment'),
       ('spam'),
       ('no_comments'),
       ('view_video'),
       ('info'),
       ('total_logins'),
       ('you_dont_hv_any_pm'),
       ('date_sent'),
       ('quicklists'),
       ('change_avatar'),
       ('uploaded_videos'),
       ('time_ago'),
       ('from_now'),
       ('private_video_error'),
       ('email_send_confirm'),
       ('name_was_empty'),
       ('invalid_email'),
       ('pelase_enter_reason'),
       ('please_enter_something_for_message'),
       ('file_size_exceeds'),
       ('vid_rate_disabled'),
       ('viewed'),
       ('sort_by_top_rated'),
       ('searching_keyword_in_obj'),
       ('no_results_found'),
       ('please_enter_val_bw_min_max'),
       ('inapp_content'),
       ('copyright_infring'),
       ('sexual_content'),
       ('violence_replusive_content'),
       ('disturbing'),
       ('other'),
       ('friend_add_himself_error'),
       ('contact_us_msg'),
       ('successful'),
       ('failed'),
       ('more_fields'),
       ('save_data'),
       ('saving'),
       ('upload_file'),
       ('upload_video_button'),
       ('photo_not_exist'),
       ('photo_success_deleted'),
       ('cant_edit_photo'),
       ('photo_rate_disabled'),
       ('need_photo_details'),
       ('embedding_is_disabled'),
       ('photo_activated'),
       ('photo_deactivated'),
       ('photo_featured'),
       ('photo_unfeatured'),
       ('photo_updated_successfully'),
       ('success_delete_file'),
       ('no_watermark_found'),
       ('watermark_updated'),
       ('upload_png_watermark'),
       ('you_dont_have_photos'),
       ('you_dont_have_fav_photos'),
       ('manage_orphan_photos'),
       ('manage_favorite_photos'),
       ('manage_photos'),
       ('you_dont_have_orphan_photos'),
       ('item_not_exist'),
       ('collection_not_exist'),
       ('selected_collects_del'),
       ('manage_collections'),
       ('manage_categories'),
       ('flagged_collections'),
       ('create_collection'),
       ('selected_items_removed'),
       ('edit_collection'),
       ('manage_collection_items'),
       ('manage_favorite_collections'),
       ('total_fav_collection_removed'),
       ('total_photos_deleted'),
       ('total_fav_photos_removed'),
       ('photos_upload'),
       ('no_items_found_in_collect'),
       ('manage_items'),
       ('add_new_collection'),
       ('update_collection'),
       ('update_photo'),
       ('no_collection_found'),
       ('photo_title'),
       ('photo_caption'),
       ('collection'),
       ('photo'),
       ('video'),
       ('pic_allow_embed'),
       ('pic_dallow_embed'),
       ('pic_allow_rating'),
       ('pic_dallow_rating'),
       ('add_more'),
       ('collect_name_er'),
       ('collect_descp_er'),
       ('collect_tag_er'),
       ('collect_cat_er'),
       ('collect_borad_pub'),
       ('collect_allow_public_up'),
       ('collect_pub_up_dallow'),
       ('collect_pub_up_allow'),
       ('collection_name'),
       ('collection_description'),
       ('collection_tags'),
       ('collect_category'),
       ('collect_added_msg'),
       ('collect_not_exist'),
       ('photos'),
       ('cat_all'),
       ('browse_photos'),
       ('photo_is_saved_now'),
       ('photo_success_heading'),
       ('share_embed'),
       ('item_added_in_collection'),
       ('object_exists_collection'),
       ('collect_broad_pri'),
       ('collect_item_removed'),
       ('collection_featured'),
       ('collection_unfeatured'),
       ('upload_right_guide_photo'),
       ('collection_deactivated'),
       ('collection_activated'),
       ('collection_updated'),
       ('cant_edit_collection'),
       ('object_not_in_collect'),
       ('object_does_not_exists'),
       ('cant_perform_action_collect'),
       ('collection_deleted'),
       ('collection_not_exists'),
       ('photo_title_err'),
       ('this_has_set_profile_item'),
       ('profile_item_removed'),
       ('make_profile_item'),
       ('remove_profile_item'),
       ('content_type_empty'),
       ('collections'),
       ('related_videos'),
       ('vdo_broadcast_unlisted'),
       ('video_link'),
       ('channel_settings'),
       ('account_settings'),
       ('allow_subscription'),
       ('allow_subscription_hint'),
       ('channel_desc'),
       ('show_my_friends'),
       ('show_my_videos'),
       ('show_my_photos'),
       ('show_my_subscriptions'),
       ('show_my_subscribers'),
       ('profile_basic_info'),
       ('profile_education_interests'),
       ('channel_profile_settings'),
       ('show_my_collections'),
       ('unsubscribe'),
       ('channel_rating_disabled'),
       ('user_activity'),
       ('you_cant_view_profile'),
       ('only_friends_view_channel'),
       ('collect_type'),
       ('add_to_my_collection'),
       ('block_users'),
       ('you_cannot_rate_own_collection'),
       ('collection_rating_not_allowed'),
       ('you_cant_rate_own_video'),
       ('you_cant_rate_own_channel'),
       ('you_cannot_rate_own_photo'),
       ('cant_pm_banned_user'),
       ('you_are_not_allowed_to_view_user_channel'),
       ('you_cant_send_pm_yourself'),
       ('video_password'),
       ('set_video_password'),
       ('video_pass_protected'),
       ('please_enter_video_password'),
       ('video_is_password_protected'),
       ('invalid_video_password'),
       ('logged_users_only'),
       ('specify_video_users'),
       ('video_users'),
       ('not_logged_video_error'),
       ('subs_email_sent_to_users'),
       ('user_has_uploaded_new_photo'),
       ('user_joined_us'),
       ('user_has_uploaded_new_video'),
       ('watch_video'),
       ('view_photo'),
       ('user_is_now_friend_with_other'),
       ('user_has_created_new_collection'),
       ('view_collection'),
       ('user_has_favorited_video'),
       ('no_activity'),
       ('you_cant_sub_yourself'),
       ('manage_my_album'),
       ('you_dont_have_permission_to_update_this_video'),
       ('share_this_type'),
       ('seperate_usernames_with_comma'),
       ('album_privacy_updated'),
       ('you_dont_have_fav_collections'),
       ('update_blocked_user_list'),
       ('no_user_associated_with_email'),
       ('pass_changed_success'),
       ('add_as_friend'),
       ('no_item_was_selected_to_delete'),
       ('playlist_items_have_been_removed'),
       ('no_playlist_was_selected_to_delete'),
       ('featured_videos'),
       ('recent_videos'),
       ('featured_users'),
       ('top_collections'),
       ('top_playlists'),
       ('load_more'),
       ('no_playlists'),
       ('featured_photos'),
       ('no_channel_found'),
       ('add_to'),
       ('player_size'),
       ('small'),
       ('medium'),
       ('large'),
       ('iframe'),
       ('embed_object'),
       ('add_to_my_favorites'),
       ('please_select_playlist'),
       ('create_new_playlist'),
       ('select_playlist'),
       ('report_video'),
       ('report_text'),
       ('comment_as'),
       ('more_replies'),
       ('photo_description'),
       ('flag'),
       ('update_cover'),
       ('unfriend'),
       ('accept_request'),
       ('online'),
       ('offline'),
       ('upload_video'),
       ('upload_photo'),
       ('admin_area'),
       ('view_channels'),
       ('my_channel'),
       ('manage_videos'),
       ('cancel_request'),
       ('no_featured_videos_found'),
       ('no_recent_videos_found'),
       ('no_collection_found_alert'),
       ('select_collection_upload'),
       ('create_new_collection_btn'),
       ('photo_upload_tab'),
       ('no_videos_found'),
       ('latest_videos'),
       ('videos_details'),
       ('option'),
       ('flagged_obj'),
       ('watched'),
       ('since'),
       ('last_Login'),
       ('no_friends_in_list'),
       ('no_pending_friend'),
       ('hometown'),
       ('city'),
       ('schools'),
       ('occupation'),
       ('you_dont_have_videos'),
       ('write_msg'),
       ('content'),
       ('no_video'),
       ('back_to_collection'),
       ('long_txt'),
       ('make_my_album'),
       ('public'),
       ('private'),
       ('for_friends'),
       ('submit_now'),
       ('drag_drop'),
       ('upload_more_videos'),
       ('selected_files'),
       ('playlist_videos'),
       ('popular_videos'),
       ('uploading'),
       ('select_photos'),
       ('uploading_in_progress'),
       ('complete_of_photo'),
       ('upload_more_photos'),
       ('save_details'),
       ('related_photos'),
       ('no_photos_found'),
       ('search_keyword_feed'),
       ('contacts_manager'),
       ('weak_pass'),
       ('create_account_msg'),
       ('get_your_account'),
       ('type_password_here'),
       ('type_username_here'),
       ('upload_vid_thumb_msg'),
       ('watch'),
       ('processing'),
       ('creating_collection_is_disabled'),
       ('creating_playlist_is_disabled'),
       ('inactive'),
       ('vdo_actions'),
       ('view_ph'),
       ('edit_ph'),
       ('delete_ph'),
       ('title_ph'),
       ('view_edit_playlist'),
       ('no_playlist_found'),
       ('playlist_owner'),
       ('playlist_privacy'),
       ('add_to_collection'),
       ('video_added_to_playlist'),
       ('report_usr'),
       ('un_reg_user'),
       ('my_playlists'),
       ('website_offline'),
       ('loading'),
       ('hour'),
       ('hours'),
       ('day'),
       ('days'),
       ('week'),
       ('weeks'),
       ('month'),
       ('months'),
       ('year'),
       ('years'),
       ('decade'),
       ('decades'),
       ('your_name'),
       ('your_email'),
       ('type_comment_box'),
       ('guest'),
       ('no_comment_added'),
       ('register_min_age_request'),
       ('select_category'),
       ('custom'),
       ('no_playlist_exists'),
       ('edit'),
       ('create_new_account'),
       ('search_too_short'),
       ('playlist_allow_comments'),
       ('playlist_allow_rating'),
       ('playlist_description'),
       ('playlists_have_been_removed'),
       ('confirm_collection_delete'),
       ('please_select_collection'),
       ('resolution'),
       ('filesize'),
       ('empty_next'),
       ('no_items'),
       ('user_recover_user'),
       ('reply_to'),
       ('mail_type'),
       ('host'),
       ('port'),
       ('user'),
       ('auth'),
       ('mail_not_send'),
       ('mail_send'),
       ('title'),
       ('show_comments'),
       ('hide_comments'),
       ('description'),
       ('users_categories'),
       ('popular_users'),
       ('channel'),
       ('confirm_del_photo'),
       ('signups'),
       ('active_users'),
       ('user_name_invalid_len'),
       ('username_spaces'),
       ('photo_caption_err'),
       ('photo_collection_err'),
       ('details'),
       ('technical_error'),
       ('inserted'),
       ('castable_status_fixed'),
       ('castable_status_failed'),
       ('castable_status_check'),
       ('castable'),
       ('non_castable'),
       ('videos_manager'),
       ('update_bits_color'),
       ('bits_color'),
       ('bits_color_compatibility'),
       ('player_logo_reset'),
       ('player_settings_updated'),
       ('player_settings'),
       ('quality'),
       ('error_occured'),
       ('error_file_download'),
       ('dashboard_update_status'),
       ('dashboard_changelogs'),
       ('dashboard_php_config_allow_url_fopen'),
       ('signup_error_email_unauthorized'),
       ('video_detail_saved'),
       ('video_subtitles_deleted'),
       ('collection_no_parent'),
       ('collection_parent'),
       ('page_upload_video_limits'),
       ('page_upload_video_select'),
       ('page_upload_photo_limits'),
       ('video_resolution_auto'),
       ('video_thumbs_regenerated'),
       ('video_allow_comment_vote'),
       ('language'),
       ('progression'),
       ('code'),
       ('admin_tool'),
       ('launch'),
       ('stop'),
       ('in_progress'),
       ('ready'),
       ('stopping'),
       ('generate_missing_thumbs_label'),
       ('generate_missing_thumbs_description'),
       ('update_castable_status_label'),
       ('update_castable_status_description'),
       ('update_bits_color_label'),
       ('update_bits_color_description'),
       ('update_videos_duration_label'),
       ('update_videos_duration_description'),
       ('need_db_upgrade'),
       ('db_up_to_date'),
       ('update_database_version_label'),
       ('update_database_version_description'),
       ('no_version'),
       ('select_version'),
       ('version'),
       ('revision'),
       ('system_info'),
       ('hosting'),
       ('info_ffmpeg'),
       ('tool_box'),
       ('info_php_cli'),
       ('info_media_info'),
       ('info_ffprobe'),
       ('info_php_web'),
       ('must_be_least'),
       ('php_cli_not_found'),
       ('cache'),
       ('enable_cache'),
       ('enable_cache_authentification'),
       ('reset_cache_label'),
       ('reset_cache_description'),
       ('nb_files'),
       ('video_file_management'),
       ('confirm_delete_video_file'),
       ('reset_video_log_label'),
       ('reset_video_log_description'),
       ('no_conversion_log'),
       ('watch_conversion_log'),
       ('conversion_log'),
       ('open_debug'),
       ('select_queries'),
       ('update_queries'),
       ('delete_queries'),
       ('insert_queries'),
       ('execute_queries'),
       ('expensive_query'),
       ('cheapest_query'),
       ('overall_stats'),
       ('base_directory'),
       ('queries'),
       ('all_queries'),
       ('total_db_queries'),
       ('total_cache_queries'),
       ('total_execution_time'),
       ('total_memory_used'),
       ('memory_usage'),
       ('everything'),
       ('display'),
       ('disable_email'),
       ('signup_success_usr_ok_description'),
       ('signup_success_usr_ok_description_no_email'),
       ('number'),
       ('confirm_delete_subtitle_file'),
       ('video_subtitles_deleted_num'),
       ('video_subtitle_management'),
       ('waiting'),
       ('option_enable_country'),
       ('option_enable_gender'),
       ('option_enable_user_category'),
       ('video_upload_disabled'),
       ('plugin_compatible'),
       ('plugin_not_compatible'),
       ('clean_orphan_files_label'),
       ('clean_orphan_files_description'),
       ('lang_restored'),
       ('language_name'),
       ('restore_language'),
       ('restore'),
       ('repair_video_duration_label'),
       ('repair_video_duration_description'),
       ('email_tester'),
       ('manage_tags'),
       ('action'),
       ('label'),
       ('tag_updated'),
       ('tag_deleted'),
       ('general'),
       ('confirm_delete_tag'),
       ('clean_orphan_tags'),
       ('clean_orphan_tags_description'),
       ('cannot_delete_tag'),
       ('tag_too_short'),
       ('tag_type'),
       ('playlist'),
       ('profile'),
       ('sign_in_with_your_cb_account'),
       ('video_is'),
       ('photo_is'),
       ('incorrect_url'),
       ('option_enable_user_firstname_lastname'),
       ('option_enable_user_relation_status'),
       ('option_enable_user_postcode'),
       ('option_enable_user_hometown'),
       ('option_enable_user_city'),
       ('option_enable_user_education'),
       ('option_enable_user_schools'),
       ('option_enable_user_occupation'),
       ('option_enable_user_compagnies'),
       ('option_enable_user_hobbies'),
       ('option_enable_user_favorite_movies'),
       ('option_enable_user_favorite_music'),
       ('option_enable_user_favorite_books'),
       ('option_enable_user_website'),
       ('option_enable_user_about'),
       ('option_enable_user_status'),
       ('video_not_available'),
       ('option_enable_video_social_sharing'),
       ('option_enable_video_internal_sharing'),
       ('option_enable_video_link_sharing'),
       ('download'),
       ('unlisted'),
       ('of'),
       ('collection_not_active'),
       ('upload_custom_thumbnail'),
       ('extracted_thumbs'),
       ('custom_thumbs'),
       ('confirm'),
       ('uploaded'),
       ('photo_tags'),
       ('collection_is'),
       ('not_found'),
       ('age_restriction'),
       ('info_age_restriction'),
       ('access_forbidden_under_age'),
       ('option_enable_age_restriction'),
       ('option_enable_user_dob_edition'),
       ('user_dob_edition_disabled'),
       ('option_enable_blur_restricted_content'),
       ('tips_enable_blur_restricted_content'),
       ('enable_global_age_restriction'),
       ('error_age_restriction'),
       ('error_age_restriction_save'),
       ('age_verification'),
       ('age_verification_text'),
       ('tips_enable_global_age_restriction'),
       ('disclaimer_return'),
       ('disclaimer_older'),
       ('edition_min_age_request'),
       ('option_enable_quicklist'),
       ('option_hide_empty_collection'),
       ('comment_disabled_for'),
       ('dev_mode'),
       ('dev'),
       ('discord_error_log'),
       ('discord_webhook_url'),
       ('discord_webhook_url_invalid'),
       ('option_enable_sitemap');

INSERT INTO `{tbl_prefix}languages_keys` (language_key)
VALUES ('admin_setting'),
       ('site_setting'),
       ('admin_username'),
       ('admin_password'),
       ('admin_email'),
       ('save_continue'),
       ('hint_admin_username'),
       ('hint_admin_email'),
       ('admin_install_info'),
       ('generate'),
       ('current'),
       ('website_configuration'),
       ('website_configuration_info'),
       ('website_title'),
       ('website_title_hint'),
       ('website_slogan'),
       ('website_slogan_hint'),
       ('website_url'),
       ('website_url_hint'),
       ('agreement'),
       ('pre_check'),
       ('permission'),
       ('database'),
       ('data_import'),
       ('continue_admin_area'),
       ('continue_to'),
       ('successful_install'),
       ('finish'),
       ('default_language'),
       ('error_server_config'),
       ('not_required_version'),
       ('category_type_unknown'),
       ('dob_required'),
       ('completed'),
       ('error_allow_photo_types'),
       ('mysql_client'),
       ('mysql_server'),
       ('option_git_path'),
       ('core_upgrade_avaible'),
       ('update_core_label'),
       ('update_core_description'),
       ('core_up_to_date'),
       ('clean_session_table_label'),
       ('clean_session_table_description'),
       ('tool_recalcul_video_file_label'),
       ('tool_recalcul_video_file_description'),
       ('recreate_thumb_label'),
       ('recreate_thumb_description'),
       ('no_logs'),
       ('show_log'),
       ('tool_started'),
       ('tool_stopped'),
       ('tool_ended'),
       ('enable_tmdb'),
       ('tmdb_token'),
       ('tmdb_token_check'),
       ('get_data_tmdb'),
       ('release_date'),
       ('poster'),
       ('import'),
       ('actors'),
       ('producer'),
       ('crew'),
       ('genre'),
       ('executive_producer'),
       ('director'),
       ('option_tmdb_get_genre'),
       ('option_tmdb_get_actors'),
       ('option_tmdb_get_producer'),
       ('option_tmdb_get_executive_producer'),
       ('option_tmdb_get_director'),
       ('option_tmdb_get_crew'),
       ('option_tmdb_get_poster'),
       ('option_tmdb_get_release_date'),
       ('option_tmdb_get_title'),
       ('option_tmdb_get_description'),
       ('tmdb_search'),
       ('movie_infos'),
       ('option_enable_video_poster'),
       ('option_enable_video_backdrop'),
       ('option_tmdb_get_age_restriction'),
       ('option_tmdb_get_backdrop'),
       ('backdrop'),
       ('thumbnail'),
       ('default_x'),
       ('option_require_x_enabled'),
       ('select_as_default_x'),
       ('enable_x_field'),
       ('update_category'),
       ('add_new_category'),
       ('edit_from_BO'),
       ('edit_from_FO'),
       ('enable_edit_button'),
       ('only_keep_max_resolution'),
       ('thumbs'),
       ('enable_tmdb_mature_content'),
       ('tmdb_mature_content_age'),
       ('access_forbidden_under_age_display'),
       ('tmdb_enable_on_front'),
       ('enabled'),
       ('disabled'),
       ('cloudflare_documentation'),
       ('incorrect_configuration_413_error'),
       ('available'),
       ('unavailable'),
       ('sse_error_features_disabled'),
       ('sse_error_please_use_php_fpm'),
       ('storage'),
       ('option_comments_censor'),
       ('option_censored_words'),
       ('separated_by_commas'),
       ('option_enable_description_censor'),
       ('option_enable_description_link'),
       ('option_enable_chunk_upload'),
       ('tips_enable_chunk_upload'),
       ('mb'),
       ('kb'),
       ('in_x'),
       ('cloudflare_upload_limit'),
       ('option_nginx_path'),
       ('file_size_cant_exceeds_x_x'),
       ('confirm_delete_video'),
       ('cancel_uploading'),
       ('pourcent_completed'),
       ('option_enable_user_profil_censor'),
       ('option_enable_playlists'),
       ('option_enable_comments_video'),
       ('option_enable_comments_photo'),
       ('option_enable_comments_collection'),
       ('option_enable_comments_channel'),
       ('thumb_regen_end'),
       ('thumb_regen_start'),
       ('db_updating'),
       ('core_updating'),
       ('launch_wip'),
       ('correct_video_categorie_label'),
       ('correct_video_categorie_description'),
       ('delete_unused_resolution_files_label'),
       ('delete_unused_resolution_files_description'),
       ('on_error'),
       ('anonymous'),
       ('anonymous_locked'),
       ('orphan'),
       ('tool_not_found'),
       ('tips_automate_launch_mode'),
       ('option_automate_launch_mode'),
       ('option_automate_launch_mode_crontab'),
       ('option_automate_launch_mode_user_activity'),
       ('option_automate_launch_mode_disabled'),
       ('frequence'),
       ('frequence_enabled'),
       ('cron_format_title'),
       ('bad_format_cron'),
       ('tool_already_launched'),
       ('success_update_tools'),
       ('automate_label'),
       ('automate_description'),
       ('datetime_synchro_error'),
       ('datetime_synchro'),
       ('automate_launch_disabled_in_config'),
       ('crontab_link_label'),
       ('copy_clipboard'),
       ('option_timezone'),
       ('unknown_type'),
       ('calc_user_storage_label'),
       ('calc_user_storage_description'),
       ('enable_storage_history'),
       ('enable_storage_history_fo'),
       ('user_current_storage'),
       ('storage_use'),
       ('storage_history'),
       ('upload_poster'),
       ('upload_backdrop'),
       ('poster_list'),
       ('backdrop_list'),
       ('posters'),
       ('backdrops'),
       ('thumbs_upload_successfully'),
       ('poster_upload_successfully'),
       ('backdrop_upload_successfully'),
       ('thumbs_delete_successfully'),
       ('backdrop_delete_successfully'),
       ('poster_delete_successfully'),
       ('manage_x'),
       ('remove_from_favorites'),
       ('interfaces'),
       ('global'),
       ('option_default_theme'),
       ('option_default_theme_light_original'),
       ('option_default_theme_light'),
       ('option_default_theme_dark'),
       ('option_custom_css'),
       ('cannot_be_own_parent'),
       ('collection_type_must_be_same_as_parent'),
       ('templates'),
       ('players'),
       ('pages'),
       ('make_featured'),
       ('make_unfeatured'),
       ('object'),
       ('objects'),
       ('view_channel_comments'),
       ('option_enable_social_networks_links_footer'),
       ('option_enable_social_networks_links_sidebar'),
       ('manage_social_networks_links'),
       ('add_new_social_network_link'),
       ('plugins'),
       ('url'),
       ('order'),
       ('icon'),
       ('validate'),
       ('title_cannot_be_empty'),
       ('url_cannot_be_empty'),
       ('icon_is_required'),
       ('confirm_delete_social_network'),
       ('missing_params'),
       ('text_confirm_comment'),
       ('collection_not_found'),
       ('edit_photo'),
       ('enable_video_view_history'),
       ('sort_by_viewed_recently'),
       ('sort_by_shorter'),
       ('sort_by_longer'),
       ('option_homepage_enable_popin_video'),
       ('option_homepage_recent_videos_display'),
       ('option_paginate'),
       ('option_slider'),
       ('option_homepage_recent_video_style'),
       ('option_modern'),
       ('option_classic'),
       ('option_list_recent_videos'),
       ('option_list_featured_videos'),
       ('tips_only_with_slider_option'),
       ('option_enable_fullwidth'),
       ('option_disable_sidebar'),
       ('option_home_display_featured_collections'),
       ('option_homepage_collection_video_style'),
       ('option_list_home_collection_videos'),
       ('option_homepage_collection_video_ratio'),
       ('option_homepage_recent_video_ratio'),
       ('option_home_display_recent_videos'),
       ('view_more'),
       ('enable_access_view_video_history'),
       ('video_list_view_video_history'),
       ('view_history'),
       ('history'),
       ('enable_channel_page'),
       ('disable_channel'),
       ('admin_account'),
       ('timezone_not_corresponding'),
       ('will_be_upload_into_collection'),
       ('configurations'),
       ('template_editor'),
       ('update_logos'),
       ('languages_settings'),
       ('email_template'),
       ('watermark_settings'),
       ('assign_default_thumb_label'),
       ('assign_default_thumb_description'),
       ('default_thumb'),
       ('user_levels'),
       ('add_collection'),
       ('permissions'),
       ('writeable'),
       ('chmod_file'),
       ('force_to_error'),
       ('tool_forced_to_error'),
       ('user_level'),
       ('administrator'),
       ('registered_user'),
       ('inactive_user'),
       ('global_moderator'),
       ('maintained_by'),
       ('channel_doesnt_exists'),
       ('user_has_been_set_as_featured'),
       ('user_has_been_removed_from_featured_users'),
       ('cant_featured_deactivate_user'),
       ('cannot_access_page'),
       ('user_cant_receive_pm'),
       ('enable_channel_page_desc'),
       ('missing_timezone'),
       ('error_format_date'),
       ('option_enable_visual_editor'),
       ('option_enable_hide_uploader_name'),
       ('tips_enable_hide_uploader_name'),
       ('wip_done'),
       ('relaunch'),
       ('movie'),
       ('series'),
       ('limit_photo_related'),
       ('admin_access_desc'),
       ('allow_video_upload_desc'),
       ('allow_photo_upload_desc'),
       ('view_video_desc'),
       ('view_photos_desc'),
       ('view_collections_desc'),
       ('view_channel_desc'),
       ('view_videos_desc'),
       ('avatar_upload_desc'),
       ('video_moderation_desc'),
       ('member_moderation_desc'),
       ('ad_manager_access_desc'),
       ('manage_template_access_desc'),
       ('group_moderation_desc'),
       ('web_config_access_desc'),
       ('view_channels_desc'),
       ('playlist_access_desc'),
       ('allow_channel_bg_desc'),
       ('private_msg_access_desc'),
       ('edit_video_desc'),
       ('download_video_desc'),
       ('admin_del_access_desc'),
       ('photos_moderation_desc'),
       ('collection_moderation_desc'),
       ('plugins_moderation_desc'),
       ('tool_box_desc'),
       ('allow_manage_user_level_desc'),
       ('allow_create_collection_desc'),
       ('allow_create_playlist_desc'),
       ('admin_access'),
       ('allow_video_upload'),
       ('allow_photo_upload'),
       ('view_photos'),
       ('view_collections'),
       ('view_channel'),
       ('view_videos'),
       ('avatar_upload'),
       ('video_moderation'),
       ('member_moderation'),
       ('ad_manager_access'),
       ('manage_template_access'),
       ('group_moderation'),
       ('web_config_access'),
       ('playlist_access'),
       ('allow_channel_bg'),
       ('private_msg_access'),
       ('edit_video'),
       ('download_video'),
       ('admin_del_access'),
       ('photos_moderation'),
       ('collection_moderation'),
       ('plugins_moderation'),
       ('allow_manage_user_level'),
       ('allow_create_collection'),
       ('allow_create_playlist'),
       ('display_featured_video'),
       ('featured_video_style'),
       ('classic'),
       ('modern'),
       ('recommended_dimension'),
       ('favicon'),
       ('tips_logo'),
       ('tips_favicon'),
       ('wrong_image_extension'),
       ('option_enable_nsfw_check'),
       ('tips_enable_nsfw_check'),
       ('tips_powered_by_ai'),
       ('warning_ai_requirements'),
       ('ai_features_disabled'),
       ('ai_setup_config'),
       ('option_nsfw_check_model'),
       ('option_nudity'),
       ('option_nsfw'),
       ('option_nudity_nsfw'),
       ('no_vid_in_playlist'),
       ('warning_php_version'),
       ('email_template_management'),
       ('email_template_management_desc'),
       ('empty_email_content'),
       ('rendered'),
       ('code_cannot_be_empty'),
       ('back_to_list'),
       ('code_already_exist'),
       ('template_set_default'),
       ('confirm_default_template'),
       ('email_variable_content'),
       ('success'),
       ('sender'),
       ('email_sender'),
       ('recipient'),
       ('email_recipient'),
       ('select_an_email'),
       ('invalid_email_recipient'),
       ('invalid_email_sender'),
       ('missing_recipient'),
       ('unknown_email'),
       ('template_dont_exist'),
       ('email_sender_address'),
       ('email_sender_name'),
       ('email_variable_website_title'),
       ('email_variable_user_username'),
       ('email_variable_user_avatar'),
       ('email_variable_date_year'),
       ('email_variable_baseurl'),
       ('email_variable_date_time'),
       ('email_variable_login_link'),
       ('email_variable_avcode'),
       ('email_variable_video_link'),
       ('email_variable_video_title'),
       ('email_variable_video_thumb'),
       ('email_variable_video_description'),
       ('email_variable_sender_message'),
       ('email_variable_subject'),
       ('email_variable_profile_link'),
       ('email_variable_request_link'),
       ('email_variable_photo_link'),
       ('email_variable_photo_thumb'),
       ('email_variable_photo_description'),
       ('email_variable_photo_title'),
       ('email_variable_collection_link'),
       ('email_variable_collection_thumb'),
       ('email_variable_collection_description'),
       ('email_variable_collection_title'),
       ('email_variable_total_items'),
       ('email_variable_collection_type'),
       ('email_variable_private_message_link'),
       ('email_variable_password'),
       ('email_variable_reset_password_link'),
       ('email_variable_comment_link'),
       ('email_variable_object'),
       ('email_variable_sender_username'),
       ('email_variable_sender_email'),
       ('email_variable_user_email'),
       ('error_mail'),
       ('email_specific'),
       ('title_email_variables'),
       ('tips_email_variables'),
       ('email_variable_logo_url'),
       ('email_variable_favicon_url'),
       ('cannot_remove_default_have_to_add_one'),
       ('this_user_blocked_you'),
       ('user_is_banned'),
       ('you_cant_share_to_yourself'),
       ('link_this_photo'),
       ('link_this_collection'),
       ('share'),
       ('enable_link_sharing'),
       ('enable_internal_sharing'),
       ('element'),
       ('motif'),
       ('reporter'),
       ('unflag'),
       ('delete_element'),
       ('unflag_and_activate'),
       ('video_flagged'),
       ('user_flagged'),
       ('collection_flagged'),
       ('playlist_flagged'),
       ('photo_flagged'),
       ('report_successful'),
       ('unflag_successful'),
       ('element_deleted'),
       ('nb_flag'),
       ('flagged'),
       ('must_update_version'),
       ('missing_email_recipient'),
       ('missing_category_report'),
       ('viewing_permission'),
       ('uploading_permission'),
       ('administration_permission'),
       ('general_permission'),
       ('website_base_url'),
       ('website_base_url_hint'),
       ('option_thumb_background_color'),
       ('enable_country_video_field'),
       ('enable_location_video_field'),
       ('enable_recorded_date_video_field'),
       ('allow_tag_space'),
       ('use_tab_tag'),
       ('orphan_file_has_been_deleted'),
       ('processing_x_files'),
       ('loading_file_list'),
       ('x_orphan_files_have_been_deleted'),
       ('option_enable_360_video'),
       ('x_fov'),
       ('video_fov'),
       ('basic_settings'),
       ('advanced_settings'),
       ('basic_settings_desc'),
       ('advanced_settings_desc'),
       ('option_enable_video_thumbs_preview'),
       ('option_video_thumbs_preview_count'),
       ('uploaded_by_x');

INSERT INTO `{tbl_prefix}languages_keys` (language_key)
VALUES ('ongoing_conversion'),
       ('mass_category_selection'),
       ('mass_broadcast_selection'),
       ('cannot_delete_not_empty_category'),
       ('option_show_collapsed_checkboxes'),
       ('changelog'),
       ('older_versions'),
       ('page_name_about_us'),
       ('page_name_privacy_policy'),
       ('page_name_terms_of_service'),
       ('page_name_help'),
       ('translation_already_exist_choose_other_name'),
       ('page_name_cant_have_space'),
       ('page_display_changed'),
       ('sort_by_most_old'),
       ('default_sort'),
       ('sort_by_featured'),
       ('sort_by_most_commented'),
       ('sort_by_most_items'),
       ('enable_categories'),
       ('launch_tool'),
       ('enable_theme_change'),
       ('enable_theme_change_tips'),
       ('option_theme_auto'),
       ('title_theme_auto'),
       ('title_theme_x'),
       ('max_video_categories'),
       ('max_video_categories_hint'),
       ('update_aspect_ratio_label'),
       ('update_aspect_ratio_description'),
       ('approve_x'),
       ('approve'),
       ('see_all_notifications'),
       ('invalid_date_format'),
       ('directory_x_is_forbidden'),
       ('nginx_vhost'),
       ('nginx_vhost_desc'),
       ('nginx_vhost_last_updated'),
       ('nginx_vhost_first_update'),
       ('nginx_vhost_no_update'),
       ('nginx_vhost_update'),
       ('nginx_vhost_updated'),
       ('access_to_front_end'),
       ('sort_by_alphabetical'),
       ('sort_by_reverse_alphabetical'),
       ('option_enable_channel_slogan'),
       ('option_enable_channel_description'),
       ('channel_slogan'),
       ('option_enable_channels_slogan_display'),
       ('option_enable_user_self_deletion'),
       ('drop_my_account'),
       ('account_deletion'),
       ('account_deletion_confirmation'),
       ('account_deletion_confirmation_info'),
       ('account_deletion_yes'),
       ('account_deleted'),
       ('reconversion_started_for_x'),
       ('resume_conversion'),
       ('selected_conversion_resumed'),
       ('conversion_not_found_x'),
       ('video_not_found_with_filename_x'),
       ('conversion_x_cannot_be_resumed'),
       ('conversion_queue_warning'),
       ('option_enable_cookie_banner'),
       ('cookies'),
       ('accept'),
       ('website_uses_cookies'),
       ('necessary_cookies_only'),
       ('cookies_and_consent'),
       ('undefined'),
       ('close'),
       ('cookie_description_quicklist'),
       ('cookie_description_phpsessid'),
       ('cookie_description_lang'),
       ('cookie_description_theme'),
       ('cookie_description_theme_auto'),
       ('cookie_description_age_restrict'),
       ('cookie_description_consent'),
       ('save_preferences'),
       ('cookie_cant_be_disabled'),
       ('accept_all'),
       ('refuse_all_optionnal'),
       ('lifetime'),
       ('no_comment'),
       ('make_public'),
       ('make_private'),
       ('created_x'),
       ('collection_made_public'),
       ('collection_made_private'),
       ('x_collections_made_public'),
       ('x_collections_made_private'),
       ('overall_statistics'),
       ('user_statistics'),
       ('video_statistics'),
       ('reported'),
       ('total'),
       ('option_photo_ratio'),
       ('option_photo_ratio_help'),
       ('option_large_photo_width'),
       ('option_crop_image'),
       ('option_photo_thumb_size'),
       ('width'),
       ('height'),
       ('option_medium_photo_size'),
       ('reset'),
       ('logo_reset'),
       ('favicon_reset'),
       ('option_max_photo_categories'),
       ('option_max_photo_categories_hint'),
       ('option_max_collection_categories'),
       ('option_max_collection_categories_hint'),
       ('request_has_been_canceled'),
       ('confirm_unfriend'),
       ('my_collections'),
       ('my_friends_collections'),
       ('public_collections'),
       ('confirm_delete_user_level'),
       ('enable_public_video_page'),
       ('enable_public_video_page_tips'),
       ('public_videos'),
       ('default_homepage_desc'),
       ('default_homepage'),
       ('homepage'),
       ('allow_public_video_page'),
       ('allow_public_video_page_desc'),
       ('userlevel_cannot_be_default'),
       ('default_userlevel_cannot_be_disabled'),
       ('userlevel_cannot_be_disabled'),
       ('cannot_be_default_until_activated'),
       ('file'),
       ('submit'),
       ('invalid_subtitle_file'),
       ('invalid_subtitle_extension'),
       ('subtitle_uploaded_successfully'),
       ('file_size_exceeded'),
       ('option_maximum_allowed_subtitle_size'),
       ('option_can_upload_subtitles'),
       ('info_upload_subtitle'),
       ('invalid_subtitle_timer'),
       ('upload_subtitles'),
       ('subtitle_already_exists'),
       ('alert_update_core_already_ongoing'),
       ('alert_update_db_already_ongoing'),
       ('alert_video_conversion_ongoing'),
       ('miscellaneous_options'),
       ('option_video_bloc_style'),
       ('add_member'),
       ('max_option_display'),
       ('display_option'),
       ('tools_seems_stuck_since_mark_as_failed'),
       ('confirm_mark_as_failed'),
       ('anonymous_stats'),
       ('anonymous_stats_hint'),
       ('unknown_task_x'),
       ('anonymous_stats_label'),
       ('anonymous_stats_description'),
       ('running_verification'),
       ('checked_user'),
       ('player_replay'),
       ('option_enable_video_remote_play'),
       ('remote_play'),
       ('remote_play_input_url'),
       ('remote_play_input_url_example'),
       ('remote_play_form_description'),
       ('remote_play_checking'),
       ('remote_play_saving'),
       ('remote_play_video_saved'),
       ('remote_play_saving_error'),
       ('remote_play_not_valid_video'),
       ('remote_play_url_not_working'),
       ('remote_play_website_not_responding'),
       ('remote_play_invalid_extension'),
       ('remote_play_invalid_url'),
       ('remote_play_invalid_step'),
       ('remote_play_invalid_duration'),
       ('option_enable_video_embed_players'),
       ('video_embed_players'),
       ('html'),
       ('confirm_delete_embed_player'),
       ('no_embed_player_to_display'),
       ('add_new_embed_player'),
       ('html_code_is_invalid'),
       ('log_file_doesnt_exist'),
       ('tool_disabled'),
       ('option_enable_multi_factor_authentification'),
       ('tips_need_confirm_email'),
       ('allow'),
       ('allowed_by_email'),
       ('confirm_email'),
       ('email_confirm_sent'),
       ('mfa_code_expired'),
       ('user_email_verify_msg'),
       ('please_enter_mfa_code'),
       ('disable'),
       ('cant_delete_only_admin'),
       ('email_variable_mfa_code'),
       ('email_confirm_last_sent_under_15_min'),
       ('multi_factor_auth_err'),
       ('cant_activate_multi_factor_auth_with_no_confirmed_email'),
       ('email_confirmation'),
       ('email_confirmation_tips'),
       ('activate_my_account'),
       ('request_activation_code'),
       ('email_confirmed'),
       ('email_already_confirmed'),
       ('email_forgot_password_sended'),
       ('forgot_password_tips'),
       ('invalid_mfa_code'),
       ('please_enter_6_digit_mfa_code'),
       ('please_fill_all_fields'),
       ('cannot_activate_auth_email_if_email_disabled'),
       ('email_mfa_hint'),
       ('enter_email_and_avcode'),
       ('reset_password'),
       ('confirm_reset_password'),
       ('cant_reset_database_not_up_to_date'),
       ('save_new_password'),
       ('new_password'),
       ('confirm_new_password'),
       ('recap_verify_failed'),
       ('if_email_exist_been_sent'),
       ('at_least_one_resolution'),
       ('file_not_found'),
       ('file_has_been_updated'),
       ('unable_to_write_file'),
       ('no_file_selected'),
       ('warning_official_cb_template'),
       ('launch_video_conversion_label'),
       ('launch_video_conversion_description'),
       ('video_is_not_convertable'),
       ('video_is_already_processing'),
       ('all_locks_deleted'),
       ('delete_conversion_lock'),
       ('no_lock_to_delete'),
       ('video_title_exceed'),
       ('install_missing_config_label'),
       ('install_missing_config_description'),
       ('install_missing_translation_label'),
       ('install_missing_translation_description'),
       ('error_missing_config_please_use_tool'),
       ('error_missing_translation_please_use_tool'),
       ('cant_perform_action_until_app_fully_updated'),
       ('base_url_required'),
       ('please_login_to_flag'),
       ('please_login'),
       ('video_is_already_waiting'),
       ('embed_player_disabled'),
       ('restrict_content_login'),
       ('confirmation_upgrade_core_php_version_require'),
       ('option_enable_allow_alias_email'),
       ('option_enable_allow_alias_email_hint'),
       ('error_alias_email_not_allowed'),
       ('option_enable_favorite_icon'),
       ('option_enable_favorite_icon_hint'),
       ('custom_thumbnail'),
       ('custom_poster'),
       ('custom_backdrop'),
       ('error_uploading_thumb'),
       ('option_keep_ratio_photo'),
       ('photo_delete_successfully'),
       ('option_keep_ratio_photo_description'),
       ('option_ratio_photo'),
       ('show_original'),
       ('bps'),
       ('hz'),
       ('fps'),
       ('option_video_thumbs_format'),
       ('you_cant_rate_own_photo'),
       ('you_cant_rate_own_collection'),
       ('collection_rate_disabled'),
       ('channel_rate_disabled'),
       ('active_template'),
       ('available_templates'),
       ('create_copy'),
       ('delete_template_confirmation'),
       ('selected_template_cannot_be_deleted'),
       ('save'),
       ('error_occurred'),
       ('saved_success'),
       ('html_files'),
       ('css_files'),
       ('select_theme_to_edit'),
       ('edit_template'),
       ('editing_template'),
       ('field_cannot_be_empty'),
       ('my_amazing_template'),
       ('author_name'),
       ('this_is_my_amazing_template'),
       ('my_amazing_website'),
       ('end_edition'),
       ('option_video_remove_black_bars'),
       ('tips_video_remove_black_bars'),
       ('option_random_video_order'),
       ('sort_by_random'),
       ('option_enable_favorite_icon_hint_photo'),
       ('option_enable_favorite_icon_hint_collection'),
       ('page_order_has_been_updated'),
       ('option_enable_profiling'),
       ('option_profiling_db_host'),
       ('option_profiling_db_name'),
       ('option_profiling_db_user'),
       ('option_profiling_db_password'),
       ('option_profiling_db_port'),
       ('option_profiling_hidden_tips'),
       ('option_profiling_tips'),
       ('option_profiling_error'),
       ('warning_profiling_requirements'),
       ('option_enable_featured_collection_hierarchy'),
       ('display_collection_hierarchy_featured_hint'),
       ('hierarchy_featured'),
       ('regenerate_all_video_thumbs_label'),
       ('regenerate_all_video_thumbs_description'),
       ('confirmation_upgrade_core_breaking_version'),
       ('maintenance_recommended'),
       ('do_want_to_update'),
       ('website_closed'),
       ('update_completed_maintenance_off'),
       ('update_incomplete'),
       ('maintenance_still_on'),
       ('ok'),
       ('live_statistics'),
       ('ongoing_videos_conversions'),
       ('online_users'),
       ('ongoing_tools'),
       ('option_enable_own_comment_rating'),
       ('comment_rate_disabled'),
       ('you_cant_rate_own_comment'),
       ('option_photo_thumbs_format'),
       ('photo_thumbs_format_tips'),
       ('keep_photo_type'),
       ('webp'),
       ('task_queue_cleaner_label'),
       ('task_queue_cleaner_description'),
       ('user_date_joined'),
       ('user_date_last_login'),
       ('user_date_last_active'),
       ('profile_views'),
       ('user_total_videos'),
       ('user_video_watched'),
       ('user_comments_made'),
       ('user_comments_received'),
       ('user_profile_rating'),
       ('user_profile_rating_count'),
       ('ip'),
       ('nb_users_are_online'),
       ('see_detail'),
       ('kick'),
       ('kick_hint'),
       ('right_now'),
       ('view_referer'),
       ('recent_activity_log'),
       ('view_online_users'),
       ('update_tmdb_info_label'),
       ('update_tmdb_info_description'),
       ('template_compatible'),
       ('template_not_compatible'),
       ('compatibility'),
       ('unknown'),
       ('option_use_backdrop_as_default_thumb'),
       ('use_backdrop_as_default_thumb'),
       ('allowed'),
       ('disallowed'),
       ('forced'),
       ('allowed_photo_extensions'),
       ('usr_background_update'),
       ('maximum_image_size'),
       ('unsupported_photo_type'),
       ('file_width_exceeds'),
       ('s_cant_exceed_s'),
       ('max_upload_size'),
       ('max_upload_size_hint'),
       ('allowed_video_extensions'),
       ('tag_too_short_dynamic'),
       ('option_random_photo_order'),
       ('likes'),
       ('dislikes'),
       ('recalcul_rates_label'),
       ('recalcul_rates_description'),
       ('option_on_delete_user'),
       ('delete_user_s'),
       ('action_logs'),
       ('show_x_logs_entries'),
       ('show_log_for_x_type'),
       ('login_only'),
       ('signup_only'),
       ('upload_video_only'),
       ('clean_all'),
       ('filter'),
       ('actions'),
       ('log_id'),
       ('user_email'),
       ('action_performed'),
       ('action_time'),
       ('confirm_clean_all_logs'),
       ('watch_a_video'),
       ('profile_update'),
       ('login_as'),
       ('add_friend'),
       ('video_favorite'),
       ('photo_favorite'),
       ('collection_favorite'),
       ('channel_comment'),
       ('video_comment'),
       ('photo_comment'),
       ('collection_comment'),
       ('option_enable_external_rate_field'),
       ('option_enable_external_ratings_field'),
       ('option_enable_external_rate_ratings_on_fo'),
       ('option_enable_external_rate_from_tmdb'),
       ('option_enable_external_ratings_from_tmdb'),
       ('external_rate'),
       ('external_ratings'),
       ('please_enter_val_bigger_than_min'),
       ('please_enter_val_smaller_than_max'),
       ('external_video_rate'),
       ('external_video_ratings');
