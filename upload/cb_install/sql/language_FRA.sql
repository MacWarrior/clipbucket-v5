...
INSERT INTO `{tbl_prefix}languages_translations` (`id_language_key`, `translation`, `language_id`)
VALUES ((SELECT id_language_key FROM `{tbl_prefix}languages_keys` WHERE `language_key` = 'error_php_version'), 'Votre version PHP actuelle (%s) est dépréciée et ne sera plus compatible avec les futures version de ClipBucketV5, veuillez mettre à jour vers à minima PHP 8.1 (8.3 recommandé)', @language_id);
