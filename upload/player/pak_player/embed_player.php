...
if(!function_exists('embed_video_src'))
{
	function embed_video_src($params)
	{
		...
		$plugins['controls']['url'] = '/player/pak_player/pakplayer.controls.swf';
		$plugins['controls']['background'] = 'url(/player/pak_player/bg.png) repeat';
		...
		if($ytcode)
		{
			$plugins['youtube']['url'] = '/player/pak_player/pakplayer.youtube.swf';
			...
			{
				$plugins['pseudo']['url'] = '/player/pak_player/pakplayer.pseudo.swf';
		...
		if(!$params['only_configs'])
			return '/player/pak_player/'.$ppfile.'?config='.json_encode($array);
