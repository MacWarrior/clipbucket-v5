...
if (!function_exists('editors_pick'))
{
    ...
    function video_manager_ep_link($vid)
    {
        if (is_video_in_editors_pick($vid['videoid']))
            return '<li><a href="' . queryString(NULL, array('remove_editor_pick', 'add_editor_pick', 'mode')) . 'remove_editor_pick=' . $vid['videoid'] . '">Remove From Editor\'s Pick</a></li>';
        else
            return '<li><a href="' . queryString(NULL, array('remove_editor_pick', 'add_editor_pick', 'mode')) . 'add_editor_pick=' . $vid['videoid'] . '">Add To Editor\'s Pick</a></li>';
    }
    ...
    function get_ep_videos($order = NULL, $limit = NULL)
    {
        ...
        $fields_arr = array(
            'v' => array('videoid', 'videokey', 'title', 'description', 'tags', 'category', 
            'views', 'date_added', 'comments_count','file_directory'),
            'u' => array('userid', 'username', 'first_name', 'last_name', 'email', 'avatar', 'avatar_url'),
            's' => array('slug'),
        );
        $the_fields = "";

        foreach ($fields_arr as $tbl => $fields)
        {
            foreach ($fields as $field)
            {
                if($the_fields)
                    $the_fields .=',';
                $the_fields .=$tbl . '.' . $field;
            }
        }
        
        $the_fields .= ','.$meta_query = $cbvid->create_meta_query();

        $query = "SELECT $the_fields FROM " . tbl('editors_picks') . " as ep ";
        $query .= " LEFT JOIN " . tbl('video') . " as v";
        $query .= " ON ep.videoid = v.videoid ";
        $query .= " LEFT JOIN " . tbl('users') . " as u";
        $query .= " ON v.userid = u.userid ";
        $query .= " LEFT JOIN " . tbl('slugs') . " as s ";
        $query .= " ON v.slug_id = s.slug_id ";
        $query .= ' LEFT JOIN ' . tbl('video_meta') . ' ON ';
        $query .= 'v.videoid = ' . tbl('video_meta.videoid');
        
        $query .= " WHERE v.status = 'Successful' AND v.active='yes' ";
        $query .= " GROUP BY v.videoid";
