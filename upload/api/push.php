...
function send_video_notification($videoId)
{
    ...
    if ($token_result) {
        foreach ($token_result as $token_value) {
            ...
            $payload['aps'] = ['alert' => "'" . $title . "' has been Featured.", 'sound' => 'default'];
            $payload['details'] = ['type' => 'featured', 'videoId' => $videoId, 'title' => $title];
        ...
        return json_encode(['err' => lang(error('single'))]);
    }
