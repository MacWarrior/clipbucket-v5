...
RUN if [ "$INSTALL_PROFILING" = "true" ]; then                (apt-get install -y --no-install-recommends php${PHP_VERSION}-sqlite3 php${PHP_VERSION}-xhprof ||         pecl install xhprof) &&         echo "extension=xhprof.so" > /etc/php/${PHP_VERSION}/mods-available/xhprof.ini &&         phpenmod xhprof  ;     fi
